...
fn init_step<E: std::error::Error + PartialEq + Eq>(
    ...
    match &mut node.node {
        ...
        Node::AnchorCreation(i) => init_step(data, document_index, i.node_index)?,
        Node::AnchorRequest(i) => {
    ...
    *data.get_mut(index) = node;
    if let Node::Document(ref i) = data.get(index).node {
        let document_anchors = i.document_anchors.data.values().copied().collect::<Vec<_>>();
