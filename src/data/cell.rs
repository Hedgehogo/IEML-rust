use super::{mark::Mark, node::Node, node_type::NodeType};
...
pub(crate) fn equal_list<'a, 'b>(
    this: (&'a ListCell, &'a Data),
    other: (&'b ListCell, &'b Data),
) -> bool {
    let mut other_iter = other.0.iter();
    this.0.iter().all(|i| {
        other_iter
            .next()
            .map_or(false, |j| this.1.get(*i) == other.1.get(*j))
    })
}

pub(crate) fn debug_list<'a, 'b>(
    this: (&'a ListCell, &'a Data),
    f: &mut Formatter<'_>,
) -> std::fmt::Result {
    write!(f, "[")?;
...
pub(crate) fn equal_map<'a, 'b>(
    this: (&'a MapCell, &'a Data),
    other: (&'b MapCell, &'b Data),
) -> bool {
    ...
    this.0.iter().all(|i| {
        other
            .0
            .get(i.0)
            .map_or(false, |j| this.1.get(*i.1) == other.1.get(*j))
    })
}

pub(crate) fn debug_map<'a, 'b>(
    this: (&'a MapCell, &'a Data),
    f: &mut Formatter<'_>,
) -> std::fmt::Result {
    write!(f, "{{")?;
...
pub(crate) fn equal_tag<'a, 'b>(
    this: (&'a TagCell, &'a Data),
    other: (&'b TagCell, &'b Data),
) -> bool {
    return this.0.tag == other.0.tag
        && this.1.get(this.0.cell_index) == other.1.get(other.0.cell_index);
}

pub(crate) fn debug_tag<'a, 'b>(
    this: (&'a TagCell, &'a Data),
    f: &mut Formatter<'_>,
) -> std::fmt::Result {
    write!(
...
pub(crate) fn equal_file<'a, 'b>(
    this: (&'a FileCell, &'a Data),
    other: (&'b FileCell, &'b Data),
) -> bool {
...
pub(crate) fn equal_take_anchor<'a, 'b>(
    this: (&'a AnchorCell, &'a Data),
    other: (&'b AnchorCell, &'b Data),
) -> bool {
    return this.0.name == other.0.name
        && this.1.get(this.0.cell_index) == other.1.get(other.0.cell_index);
}

pub(crate) fn debug_take_anchor<'a, 'b>(
    this: (&'a AnchorCell, &'a Data),
    f: &mut Formatter<'_>,
) -> std::fmt::Result {
    write!(
...
pub(crate) fn debug_get_anchor<'a, 'b>(
    this: (&'a AnchorCell, &'a Data),
    f: &mut Formatter<'_>,
) -> std::fmt::Result {
...
pub(crate) fn equal_data<'a, 'b>(
    this: (&'a DataCell, &'a Data),
    other: (&'b DataCell, &'b Data),
) -> bool {
    match (this.0, other.0) {
        ...
        (DataCell::TakeAnchor(i), DataCell::TakeAnchor(j)) => {
            equal_take_anchor((i, this.1), (j, other.1))
        }
