...
impl<'a> BasicNode<'a> {
    ...
    pub fn clear_step(&self) -> Option<Self> {
        use clear::*;
        clear_step::<(Tag, File, TakeAnchor, GetAnchor)>(*self)
    }

    /// Gets a child node if the node type is T.
    pub fn clear_step_advanced<T: clear::ClearStepType>(&self) -> Option<Self> {
        use clear::*;
        clear_step::<T>(*self)
    }
    ...
    pub fn clear(&self) -> Self {
        use clear::*;
        clear::<(Tag, File, TakeAnchor, GetAnchor)>(*self)
    }

    /// Recursively gets a child node, excluding T.
    pub fn clear_advanced<T: clear::ClearStepType>(&self) -> Self {
        use clear::*;
        clear::<T>(*self)
    }
    ...
    pub fn clear_file(&self) -> Self {
        use clear::*;
        clear::<(Tag, TakeAnchor, GetAnchor)>(*self)
    }
    ...
    pub fn clear_get_anchor(&self) -> Self {
        use clear::*;
        clear::<(Tag, File, TakeAnchor)>(*self)
    }

    fn make_error<T>(&self, error: T) -> marked::WithMarkError<T> {
    ...
    pub fn file_anchors(&self) -> Result<Anchors, marked::AnotherTypeError> {
        let clear = self.clear_file();
        match clear.cell() {
    ...
    /// Gets the list data.
    pub fn list_iter(&self) -> Result<BasicListIter<'a>, marked::AnotherTypeError> {
        let clear = self.clear();
        match clear.cell() {
    ...
    /// Gets the map data.
    pub fn map_iter(&self) -> Result<BasicMapIter<'a>, marked::AnotherTypeError> {
        let clear = self.clear();
        match clear.cell() {
    ...
    ///
    /// * `index` Index.
    pub fn at<T: NodeIndex<'a>>(&self, index: T) -> Result<Self, T::Error> {
...
impl<'a> PartialEq for BasicNode<'a> {
